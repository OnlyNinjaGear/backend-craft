...
Status: observed (case-triage pipeline's own run, 2026-07-19)
Triggered by: a CI/CD job (agent or scripted) that grants `permissions: pull-requests: write` at the workflow level and then calls `gh pr create` (or the equivalent GitHub REST API `POST /repos/{owner}/{repo}/pulls`) using the automatic `GITHUB_TOKEN`.
Model failure: treats the workflow-level `permissions:` block as sufficient authorization. It is necessary but not sufficient: a separate repository/organization setting -- "Allow GitHub Actions to create and approve pull requests", off by default -- gates PR creation independently of the job's own declared permissions, and a workflow cannot see or change this setting from inside the run. When the job also does not check the outcome of `gh pr create` and continues past it, the run concludes `success` while its entire terminal deliverable -- the PR -- never existed.
Blast radius: an unattended agent pipeline can run for days doing real, correct work (branch, commits, tests) and never produce output; the operator only discovers it by noticing an empty PR list, since every dashboard/log shows green.
Detect: workflow YAML declares `permissions: pull-requests: write` and calls `gh pr create` / the pulls API without (a) a preflight check of the repo/org-level Actions PR policy or (b) asserting after the call that a PR actually exists for the pushed branch.
Safe pattern: two independent checks, neither substitutes for the other. (1) Preflight: query `GET /repos/{owner}/{repo}/actions/permissions/workflow` and hard-fail before doing any work if `can_approve_pull_request_reviews` is false, so the run fails fast and loud instead of burning a full pipeline for nothing. (2) Postflight: after calling `gh pr create`, do not trust a clean return alone -- list PRs for the pushed branch (`gh pr list --head <branch>`) and fail the job if none exists, so no code path can call the job successful while its side effect is missing. Generalizes beyond GitHub: any two-layer permission model (inner grant + outer policy) needs both a preflight capability check and a postflight existence assertion on the terminal side effect.
Verifier: `tests/cards/infra_gha_pr_permission_toggle_silent_fail.py` + `tests/cards/test_infra_gha_pr_permission_toggle_silent_fail.py` mock a GitHub API client with independent `workflow_permissions_write` (job-level) and `repo_allows_actions_pr` (repo/org toggle) flags and prove: PR creation only succeeds when both are true; a pipeline that only checks the job-level flag and swallows the creation error reports `"success"` with zero PRs created when the repo flag is false; a pipeline with preflight + postflight fails loudly in the same scenario, and the postflight check alone still catches a creation call that returns cleanly without actually producing a PR. Run: `python3 tests/cards/infra_gha_pr_permission_toggle_silent_fail.py` and `python -m pytest tests/cards/test_infra_gha_pr_permission_toggle_silent_fail.py -q`.
Escape hatch: workflows that only push commits/tags and never create a PR (no terminal side effect to verify); manual/human-triggered PR creation, where a human already sees the failure directly in their terminal.
Sources: GitHub Docs -- "Managing GitHub Actions settings for a repository", section on allowing GitHub Actions to create and approve pull requests (documented as disabled by default at the repository/organization level, independent of workflow-level `permissions:`); GitHub REST API docs -- Actions permissions, `GET /repos/{owner}/{repo}/actions/permissions/workflow` (`can_approve_pull_request_reviews` field). Field name and default-off behavior corroborated by this run's own `gh api` output as reported in the source issue; not independently re-fetched from GitHub Docs in this run (no network/WebFetch access in this pipeline invocation) -- flag for confirmation against current docs before promoting past `observed`.
